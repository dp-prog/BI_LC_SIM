...
static const char *TAG_SIM_Timestamp = "SIM800";
#include "gsm.h"
...
//  esp_log_write(ESP_LOG_INFO, TAG_SIM_Timestamp, "Read UART start - ", esp_log_timestamp());
  int len = uart_read_bytes(uart_num, (uint8_t *)out, BUF_SIZE, 10000 / portTICK_RATE_MS);
  esp_log_early_timestamp();
//  esp_log_write(ESP_LOG_INFO, TAG_SIM_Timestamp, "Read UART start - ", esp_log_timestamp());
  if (len > 0) {
    return out;
